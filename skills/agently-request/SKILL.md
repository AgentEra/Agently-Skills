...
- In 4.1.4.8, `.auto_continue()` is a conditional output-delivery setting;
  `.ensure_long_output()` remains its compatibility alias. Neither selects an
  Execution mode. `LongContent` declarations select proactive long-form
  production. Read [output-control.md](references/output-control.md) for
  plain-text and structured-string support, completion evidence and limits.
