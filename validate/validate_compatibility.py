...
def main(*, require_remote_archives: bool = False) -> None:
    ...
    for label, text in (("readme", readme), ("readme_cn", readme_cn)):
        check(
            f"{label}_version",
            f"`{aligned_version}`" in text,
            "README states the current framework alignment",
            failures,
            passes,
        )
        check(f"{label}_catalog", "seven-skill" in text or "七个 Skill" in text, "README states the seven-skill catalog", failures, passes)
